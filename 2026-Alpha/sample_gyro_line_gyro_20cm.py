#sample_gyro_line_gyro.pyからlap2_3の距離を200mmに変更
#sample_gyro_line_gyro.pyからlap2_4の距離を2400mmに変更
...
def build_behaviour_tree() -> BehaviourTree:
    ...
    lap2_1 = Parallel(name="lap2_1", policy=ParallelPolicy.SuccessOnOne())#parallelでmemory=trueは持っていないが、sequenceに組み込むことでparallelの結果も覚えられる。
    ...
    lap2_4 = Parallel(name="lap2_4", policy=ParallelPolicy.SuccessOnOne())#カーブ箇所のライントレース
    lap2_5 = Parallel(name="lap2_5", policy=ParallelPolicy.SuccessOnOne())#カーブ終了後、青マーカーまでジャイロ
    ...
    square.add_children(
     ...
     #lap2_4（床の線を見ながら通常エッジをトレースし、3つ目の黄色△までライントレース
     #2250mmくらいと仮定
     #2400mmに変更してみた!!!
    lap2_4.add_children(
        [
            TraceLine(name="sensor trace normal edge", target=TRACELINE_TARGET_V, power=40,
                pid_p=0.55, pid_i=0.0000009, pid_d=0.015, trace_side=TraceSide.NORMAL),
            IsDistanceEarned(name="check distance", delta_dist=2400),
        ]
    )

    #lap2_5 カーブ終了後から青いマーカーまでジャイロ走行
    #900mmと仮定
    #絶対角度はゼロ？
    lap2_5.add_children(
        [
