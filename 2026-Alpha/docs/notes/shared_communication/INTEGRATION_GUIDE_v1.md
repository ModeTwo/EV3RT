# Hint取得・経路計算・経路実行 担当者向け組込みガイド v1
...
| 経路実行 | ラズパイ・robot_program/features/execute_strategy.py | ready確認後にcontext.strategyを参照し、lap_numberの周回だけ実行 | TCP受信、JSON解析、PC接続待ち |
| 通信・統合 | shared_communication/、wireless_device/transport.py、No.11、strategy_tree.py等 | 上記3者の間を接続 | 認識、復号、経路生成、モーター制御 |
...
| 順番 | 処理 | 実施する側 |
|---|---|---|
| 1 | alpha.pyから通常のキャリブレーション・タッチ待ちを実施 | 既存起動処理 |
| 2 | 競技ミッション開始時にTCP待受けを開始。PCは接続を再試行できる | 通信部 |
| 3 | Hint1を取得しcontext.hint1へ保存 | 取得担当 |
| 4 | Hint2を取得しcontext.hint2へ保存 | 取得担当 |
| 5 | 両方が非空になったBT周期で送信要求を作成。通信スレッドが送信 | 通信部 |
| 6 | 受信したHint辞書で担当者のcalculate関数を呼ぶ | PC通信部 |
| 7 | 復号・経路計算を行い、指令リストをreturnする | 計算担当 |
| 8 | 戻り値をJSONとして返信 | PC通信部 |
| 9 | context.strategyへ保存し、strategy_statusをreadyにする | ラズパイ通信部 |
| 10 | No.11で停止し、readyなら次へ進む。pendingなら待つ | 既存受信Feature |
| 11 | 各周回の補正処理の後、No.12で該当周回の指令を実行 | 実行担当 |
...
- 未取得時はNoneのままにする。
- 空文字やダミー値で「取得済み」にしない。
- Hint2の暗号文字列を、そのまま保存する。復号はPC側の担当範囲。
...
PCの呼出し口は、引数1個の通常の同期関数。

```python
# team_route.pyの接続口例。計算処理は担当者が実装する。
def calculate(hints: dict) -> list[dict]:
    # hints["hint1"]: 未復号文字列
    # hints["hint2"]: 未復号文字列
    # hints["course"]: "left"または"right"
    # hints["laps"]: 1～3
    raise NotImplementedError("Implement route calculation here")
```
...
PCの2026-Alphaをカレントディレクトリにする。
team_route.pyをその直下へ置く例：

```sh
python -m wireless_device --host RASPBERRY_PI_IP --planner team_route:calculate
```

パッケージ内の関数なら、例としてmy_planner.route:calculateのように指定できる。
共有するのは計算モジュール名と関数名でよく、通信コードを書き換える必要はない。

--plannerを省略すると既存WirelessDeviceApplication.prepare_strategyが呼ばれる。
既存のダミー計算は空リストを返すため、そのままでは成功しない。

### 4.3 キー入力・計算時間・再送

- キー入力・復号・計算の実装は計算担当側で管理する。
- キー入力は事前に完了させる。Hintを受け取ってから人の入力を待つ構成は避ける。
- --plannerはモジュールをimportして関数を呼ぶ方式なので、そのモジュールのif __name__ == "__main__":以下は実行されない。事前初期化の呼出し場所を忘れないこと。
...
[No.12](../../../robot_program/features/execute_strategy.py)のbuild_execute_strategy(context, config, lap_number)が実装箇所。
[ETラリー工程](../../../robot_program/phases/et_rally.py)はすでに次の順で呼び出す。

1. No.11：受信結果待ち
2. 1周目の補正→No.12(lap_number=1)
3. 2周目の補正→No.12(lap_number=2)
4. 3周目の補正→No.12(lap_number=3)

実際の周回数はconfig.et_rally_lapsによる。No.12自身でさらに3周ループを作らない。
...
- 起動時：contextとlap_numberを保持する実行ノードを作る。
- 実行開始時（initialise等）：readyを確認し、受信済みSEQを読み出す。
- 内容を検証して該当周回を選び、既存Behaviorを使った実行単位を準備する。
...
context.strategy_statusを書き換えてreadyにしたり、受信リストをpopして他周回分を壊したりしない。
...
| 項目 | 現状・確認内容 |
|---|---|
| 指令種別とキー | type、angle_deg、distance_mmは通信テストの例。正式な走行仕様としては未確定 |
| 周回の区切り | 1つのlistへ全周回を含める場合、どの指令が何周目か識別が必要 |
| 絶対／相対角度 | 基準方位とtarget_typeの対応を合意 |
...
端末1：
```sh
python -m shared_communication.demo_robot --hint1 "11,22" --hint2 "test-hint2"
```
...
test-hint2は復号用の実データではないため、本物の計算関数の正常系試験には使わない。

### B. PCの計算関数だけ接続（モーターなし）

Aの端末2を--planner team_route:calculateへ変更する。
端末1には担当者が用意した実際に解析できるHint組を渡す。
...
ラズパイ：
```sh
python -m shared_communication.demo_robot --host 0.0.0.0 --hint1 "11,22" --hint2 "test-hint2"
```
