...
# --- 直線区間（あなたの指定） ---
straight_segments = [(0,1), (1,2), (6,7), (26,27)]

# --- スプライン補間器を作成（曲線区間のみ） ---
# 曲線区間だけ抽出
mask = np.ones(len(S), dtype=bool)
for a, b in straight_segments:
    mask[a:b+1] = False

S_curve = np.array(S)[mask]
Theta_curve = np.array(Theta)[mask]

# スプライン補間器（曲線区間）
spline_theta = CubicSpline(S_curve, Theta_curve, bc_type='natural')
...
def heading_at(s):
    ...
    for a, b in straight_segments:
        if S[a] <= s <= S[b]:
            # 線形補間
            t = (s - S[a]) / (S[b] - S[a])
            return Theta[a] + t * (Theta[b] - Theta[a])

    # --- 曲線区間はスプライン補間 ---
    return float(spline_theta(s))



class CourseShapeGyro(Behaviour):
    """
    累積距離 s に応じて heading_at(s) が返す角度へ向けて
    ジャイロPIDで走行するノード。
    つまり「コース形状そのまま」を再現する走行。
    """
    ...
    def update(self) -> Status:
        ...
        if self.last_log_time is None or time.time() - self.last_log_time >= 1.0:
            self.logger.info("%+06d %s.s=%.1f target=%.1f current=%d"
                             % (s, self.__class__.__name__, s, target_heading, current_heading))
            self.last_log_time = time.time()
...
def build_behaviour_tree() -> BehaviourTree:
    ...
    start_to_lap.add_children(
        [
            ...
            IsDistanceEarned(name="check distance", delta_dist=5202.0),
        ]
    )
