...
def build_behaviour_tree() -> BehaviourTree:
    ...
    # 1. センサー等の初期化
    # 2. タッチセンサーが押されるまで待機
    # 3. ライントレースしながら青ラインを検知
    # 4. 青ラインを検知したら停止
    # 5. 停止状態を維持
    root = Sequence(name="touch -> trace -> blue -> stop", memory=True)

    # ライントレースと青色検知を同時に実行する。
    # IsColorDetected が SUCCESS になった時点で、この Parallel も SUCCESS になる。
    ...
    root.add_children(
        [
            ...
            IsTouchOn(name="wait for touch"),
            trace_until_blue,
            StopNow(name="stop at blue line"),
            TheEnd(name="end"),
        ]
    )
