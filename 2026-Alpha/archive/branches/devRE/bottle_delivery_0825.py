#8/25memo
#land3（ボトル置いて、同じ距離戻る処理）を追加した
"""
青色と黄色だったら、バックしたあと(lap3）、ターンして(180度）、赤い四角まで進む（距離ｈそれぞれ測る）
赤色だったら、バックした後(lap3)なにもしない
何色であっても、ここからは同じ処理で、270°？ターンして、ETラリー開始位置につく
"""

"""
仕組み（最短理解）
CatchBottle が g_bottle_color に記録した色を
HasCaughtBottle が読み取って、
その色に応じて case_yellow → case_blue → case_red の順で分岐する。
"""
import sys
import argparse
import time
...
def build_behaviour_tree() -> BehaviourTree:
    ...
    before_catch = Parallel(name="run to catch", policy=ParallelPolicy.SuccessOnOne())
    ...
    land3 = Parallel(name="land3", policy=ParallelPolicy.SuccessOnOne())
    case_red_before_ETrally = Selector(name="case red before rally", memory=True)
    case_blue_and_yellow_before_ETrally = Sequence(name="case blue and yellow before rally",memory=True)
    judge_blue_or_yellow = Selector(name="judge blue or yellow",memory=True)
    case_yellow_before_ETrally = Sequence(name="case yellow before rally", memory=True)
    preparation_before_ETrally = Sequence(name="preparation before rally", memory=True)
    ...
    #ここの距離を変える
    before_catch.add_children(
        [
            RunByGyro(name="run to catch", target=0, power=33,
    ...
    land3.add_children(
        [
            RunByGyro(name="back from  landing zone", target=90, power=33,
                pid_p=1.1, pid_i=0.1, pid_d=0.03, target_type=HeadingType.ABSOLUTE),
            IsDistanceEarned(name="check distance", delta_dist = 250),
        ]
    )
    ...
    case_red_before_ETrally.add_children(
        [
            HasCaughtBottle(name="check if bottle is RED",
                color=BottleColor.RED),
            case_blue_and_yellow_before_ETrally,
        ]
    )
   
    case_blue_and_yellow_before_ETrally.add_children(
       [
            SpinAround(name="spin by 180 degrees", target=180, max_power=55, min_power=MIN_POWER,
                pid_p=0.4, pid_i=0.001, pid_d=0.03, target_type=HeadingType.ABSOLUTE),
            RunByGyro(name="back from  landing zone", target=180, power=33,
                pid_p=1.1, pid_i=0.1, pid_d=0.03, target_type=HeadingType.ABSOLUTE),
            IsDistanceEarned(name="check distance", delta_dist = 50),
            judge_blue_or_yellow,
       ]
   )

    #BLUEだったらなにもしない。
    judge_blue_or_yellow.add_children(
        [
            HasCaughtBottle(name="check if bottle is BLUE",
                color=BottleColor.BLUE),
            case_yellow_before_ETrally,
        ]
    )
    case_yellow_before_ETrally.add_children(
       [
            RunByGyro(name="case_yellow_before_ETrally", target=180, power=33,
                pid_p=1.1, pid_i=0.1, pid_d=0.03, target_type=HeadingType.ABSOLUTE),
            IsDistanceEarned(name="check distance", delta_dist = 50),
       ]
   )
   
    preparation_before_ETrally.add_children(
       [
            SpinAround(name="spin by 270 degrees", target=270, max_power=55, min_power=MIN_POWER,
                pid_p=0.4, pid_i=0.001, pid_d=0.03, target_type=HeadingType.ABSOLUTE),
       ]
   )
    ...
    root.add_children(
        [
            ...
            case_yellow,
            land,
            case_red_before_ETrally,
            preparation_before_ETrally,
            StopNow(name="stop"),
            TheEnd(name="end"),
        ]
    )
