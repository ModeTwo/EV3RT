...
def build_behaviour_tree() -> BehaviourTree:
    ...
    #lap2_1（ライントレース。段階的に速度上げるために150mmまではpower=33でゆっくり進む）
    lap2_1.add_children(
        [
     ...
     #lap2_2（ライントレース。3つ目の黄色△までライントレース）2450mmとした.布の切れ目まで
    lap2_2.add_children(
        [
            TraceLine(name="sensor trace normal edge", target=TRACELINE_TARGET_V, power=50,
                pid_p=0.55, pid_i=0.0000009, pid_d=0.015, trace_side=TraceSide.NORMAL),
            IsDistanceEarned(name="check distance", delta_dist=2450),
        ]
    )

    #lap2_3 (ジャイロ走行。青いマーカーを超えるくらいまで）83cmくらい?布の切れ目から、青マーカー超えて赤い線延長まで
    lap2_3.add_children(
        [
            RunByGyro(
                name="run straight",
    ...
    lap3.add_children(
        [
            RunByGyro(name="run straight to catch the bottle", target=0, power=33,
