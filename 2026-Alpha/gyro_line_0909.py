...
def build_behaviour_tree() -> BehaviourTree:
    ...
    #lap2_1（ライントレース。100mmまではpower=33でゆっくり進む）30cmにした9月9日
    lap2_1.add_children(
        [
     ...
     #lap2_2（ライントレース。カーブ部分）246cm→226cmにした9月9日
    lap2_2.add_children(
        [
            TraceLine(name="sensor trace normal edge", target=TRACELINE_TARGET_V, power=60,
                pid_p=0.55, pid_i=0.0000009, pid_d=0.07, trace_side=TraceSide.NORMAL),
            IsDistanceEarned(name="check distance", delta_dist=2260),
        ]
    )

    #lap2_3（ライントレース。LAP直線の直線部分）70cm
    lap2_3.add_children(
        [
            TraceLine(name="sensor trace normal edge", target=TRACELINE_TARGET_V, power=60,
                pid_p=0.10, pid_i=0.0000009, pid_d=0.08, trace_side=TraceSide.NORMAL),
