...
class DetectBottleColorWhileMoving(Behaviour):
    """
    ライントレースで走行しながらボトル色を認識する。
    ...
    def __init__(
        ...
        super(DetectBottleColorWhileMoving, self).__init__(name)
    ...
    def update(self) -> Status:
        ...
        if not self.running:
            ...
            # カメラをボトル認識モードにする
            g_video.set_target_interested(
                TargetInterested.BOTTLE
            )

            self.logger.info(
                "%+06d %s.bottle detection while moving started"
                % (
                    g_plotter.get_distance(),
                    self.__class__.__name__
                )
        ...
        # カメラからボトル情報を取得
        # ------------------------------------------
        insight, color, bcx, btheta, bbottom, barea, in_blind = \
            g_video.get_bottle_stamped()
        ...
        if self._hits >= self.min_frames:
            ...
            print(
                " -- Bottle color detected while moving: %s"
                % color.name
            )
